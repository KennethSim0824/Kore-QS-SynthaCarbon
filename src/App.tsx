...
export default function App() {
  ...
  const isVideoSource = isCameraActive || fileType?.startsWith('video/');

  const refreshCameraDevices = useCallback(async () => {
    if (!navigator.mediaDevices?.enumerateDevices) {
      setCameraStatus('Camera API is unavailable in this browser');
      return;
    }
    ...
    try {
      ...
      setCameraStatus(videoInputs.length > 0 ? 'Camera ready' : 'No camera detected');
    } catch (error) {
      console.error('Unable to enumerate cameras:', error);
      setCameraStatus('Unable to read camera list');
    }
  }, []);
  ...
  const startCamera = async () => {
    ...
    stopCamera();
    setPreviewUrl(null);
    setFileType('camera/live');
    setDetections([]);
    lastInferenceTime.current = 0;

    try {
      const videoConstraint: MediaTrackConstraints = selectedCameraId
        ? { deviceId: { exact: selectedCameraId }, width: { ideal: 1280 }, height: { ideal: 720 }, frameRate: { ideal: 30, max: 30 } }
      ...
      setCameraStatus('Live camera running');
    } catch (error) {
  ...
  React.useEffect(() => {
    if (!isCameraActive || !videoRef.current || !cameraStreamRef.current) return;

    videoRef.current.srcObject = cameraStreamRef.current;
    videoRef.current.play().catch((error) => {
      console.error('Camera playback failed:', error);
      setCameraStatus('Camera started, but playback was blocked');
    });
  }, [isCameraActive]);
  ...
  React.useEffect(() => {
    if (useLocalModel && isVideoSource && videoRef.current && overlayCanvasRef.current) {
  ...
  }, [previewUrl, isVideoSource, useLocalModel]);
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => !isCameraActive && setUseLocalModel(!useLocalModel)}
                disabled={isCameraActive}
                className={cn(
                  "px-3 py-1 text-[10px] font-semibold rounded-full border transition-all cursor-pointer disabled:cursor-not-allowed disabled:opacity-60",
                  useLocalModel
                    ? "bg-[#1A1A1A] text-white border-[#1A1A1A]"
                    : "bg-white text-[#737373] border-cf-border hover:border-[#1A1A1A]"
                )}
              >
                {isCameraActive ? "Local Model (Live Camera)" : useLocalModel ? "Local Model (best.onnx)" : "Gemini Vision AI"}
              </button>
            </div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div
              ...
              className="group relative flex flex-col items-center justify-center p-10 border border-cf-border rounded-lg bg-cf-surface hover:border-cf-green/50 transition-all cursor-pointer overflow-hidden shadow-2xl min-h-[260px]"
            >
              ...
              <div className="flex flex-col items-center gap-4 text-center relative z-10">
                <div className="p-4 rounded-full bg-cf-bg border border-cf-border group-hover:bg-cf-green/10 group-hover:border-cf-green/30 transition-all">
                  {isAnalyzing && !isCameraActive ? <Cpu className="w-8 h-8 text-cf-green animate-spin" /> : <Upload className="w-8 h-8 text-gray-600" />}
                </div>
                <div className="space-y-1">
                  <p className="text-[15px] font-medium text-white">
                    {isAnalyzing && !isCameraActive ? "Initiating Grid Diagnostic..." : "Ingest Telemetry Data"}
                  </p>
                  <p className="text-[13px] text-gray-500">
                    Upload .mp4, .webp, .png, or .jpg footage
                  </p>
                </div>
              </div>
            </div>

            <div className="relative flex flex-col justify-between p-8 border border-cf-border rounded-lg bg-cf-surface shadow-2xl min-h-[260px] overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-br from-cf-blue/5 to-transparent pointer-events-none" />
              <div className="relative z-10 space-y-6">
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-2">
                    <div className="text-[11px] font-semibold text-[#737373] uppercase tracking-[0.1em]">Real-Time Camera</div>
                    <p className="text-[13px] text-gray-500 leading-relaxed">
                      Use a USB webcam, laptop camera, or phone camera exposed as a webcam.
                    </p>
                  </div>
                  <div className="p-3 rounded-full bg-cf-bg border border-cf-border">
                    <Smartphone className="w-6 h-6 text-cf-blue" />
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex gap-2">
                    <select
                      ...
                      className="min-w-0 flex-1 bg-cf-bg border border-cf-border rounded px-3 py-2 text-[12px] text-white outline-none focus:border-cf-blue"
                      disabled={isCameraActive}
                    >
                      {cameraDevices.length > 0 ? cameraDevices.map((device, index) => (
                    ...
                    <button
                      ...
                      className="h-9 w-9 flex items-center justify-center rounded border border-cf-border bg-cf-bg text-gray-400 hover:text-white hover:border-cf-blue transition-colors"
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2 bg-cf-blue text-white text-[12px] font-bold rounded hover:bg-cf-blue/90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors uppercase tracking-wider"
                    >
                    ...
                    <button
                      ...
                      onClick={stopCamera}
                      disabled={!isCameraActive}
                      className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2 border border-white/10 text-white text-[12px] font-medium rounded hover:bg-white/5 disabled:opacity-40 disabled:cursor-not-allowed transition-colors uppercase tracking-wider"
                    >
                ...
                {previewUrl || isCameraActive ? (
                  <>
                    ...
                    <div className="absolute inset-0 pointer-events-none">
                      ...
                      {!isVideoSource && detections.map((d, i) => (
                        <motion.div
                          key={i}
