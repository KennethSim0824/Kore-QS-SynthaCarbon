...
- Real-time camera input from a laptop webcam, USB/external webcam, or phone camera exposed to the laptop as a webcam
...
4. Or use **Real-Time Camera**:
   - Plug in an external webcam and select it from the camera list
   - To use a phone camera on the laptop, connect it as a system webcam first (for example Continuity Camera, DroidCam, Iriun, Camo, or another phone-as-webcam app), then select that camera in the app
   - To use the phone directly, open the deployed HTTPS app on the phone and press **Start Live**; the app requests the rear/environment camera when available
   - Camera access works on `localhost` during development; deployed GitHub/hosting builds must be served over HTTPS for browser camera permission
5. The system will:
